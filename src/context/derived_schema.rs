        ...
        if let schema::Definition::TypeDefinition(schema::TypeDefinition::Object(o)) = def {
            ...
            for f in derived_fields {
                ...
                if context.derived_fields.contains_key(entity_type) {
                    let mut derived_field =
                        context.derived_fields.get(entity_type).unwrap().clone();

                    if !derived_field.contains_key(&f.name) {
                        derived_field.insert(f.name.clone(), (clean_field_type, field.clone()));
                    }
                } else {
                    let mut h: HashMap<String, (String, String)> = HashMap::new();
                    h.insert(f.name.clone(), (clean_field_type, field.clone()));
                    context.derived_fields.insert(entity_type.to_string(), h);
                }
            }
        }
    });
